...
11. **默认筛选器设置（v0.0.3）：**
    - **位置：** 设置页面 → "时间线设置" → "默认筛选条件"
    - **选项：** 全部/未读/已读（工厂默认：未读）
12. **并发刷新设置（v0.0.4）：**
    - **位置：** 设置页面 → "刷新性能"
    - **功能：** 调整同时刷新的订阅源数量（1-5，默认 3）
    - **性能影响：** 高并发数加快刷新速度，但增加资源消耗
    - **实现：** 使用 Semaphore 限制并发数，配合 Kotlin Coroutines async/awaitAll
13. **智能滚动优化（v0.0.4）：**
    - **行为：** 仅当刷新后有新文章时才自动滚动到列表顶部
    - **目的：** 避免无新文章时的不必要滚动，保持阅读位置
    - **实现：** TimelineViewModel 判断 `totalNewArticles > 0` 才发送 ScrollToTop 事件
14. **精简刷新通知（v0.0.4）：**
    - **变更：** 移除单源刷新完成时的 Toast 提示
    - **保留：** 仅保留最终汇总通知（"刷新完成,共更新 N 篇新文章"）
    - **目的：** 减少 Toast 干扰，提升阅读体验
    - **生效时机：** 应用启动时使用设置的默认筛选器
    - **手动切换：** 用户在时间线页面手动切换筛选器不影响默认设置
    - **持久化：** 使用 SharedPreferences 保存，重启应用后自动恢复
12. **刷新进度反馈（v0.0.3）：**
    - **单源提示：** 每个订阅源刷新完成显示 Toast（"{源名}: 已更新 X 篇新文章"）
    - **总计提示：** 所有源刷新完成显示汇总（"刷新完成,共更新 N 篇新文章"）
    - **刷新策略：** 串行执行，每个源刷新完立即更新 UI（增量刷新）
