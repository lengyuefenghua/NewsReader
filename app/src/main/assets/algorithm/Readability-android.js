/**
 * ============================================================================
 * Readability for Android WebView - 独立注入版本
 * ============================================================================
 *
 * 【原理说明】
 *
 * Readability 是一个内容提取算法，用于从网页中自动识别并提取主要文章内容，
 * 去除导航栏、广告、侧边栏等干扰元素。其核心原理包括：
 *
 * 1. 元素评分系统：
 *    - 遍历 DOM 树，为每个元素打分
 *    - 考虑因素：文本长度、链接密度、标点符号数量、类名/ID 名称
 *    - 高分元素更有可能包含文章内容
 *
 * 2. 不可能候选识别：
 *    - 通过正则表达式识别广告、导航、评论区等元素
 *    - 排除带有特定类名/ID 的元素（如 "sidebar", "footer", "ad-"）
 *
 * 3. 内容清理：
 *    - 移除脚本、样式、表单等非内容元素
 *    - 清理多余的嵌套 div，转换为语义化标签（p, h1-h6）
 *    - 保留图片、视频等媒体内容及其说明文字
 *
 * 4. 元数据提取：
 *    - 从 JSON-LD、meta 标签、页面结构中提取标题、作者、发布时间
 *    - 支持多种格式的日期解析
 *
 * 【参考来源】
 *
 ...
 *
 * - Arc90 Readability (原始版本)
 *   http://code.arc90.com/readability/
 *
 * - 学术论文："Boilerplate Detection using Shallow Text Features"
 *   https://dl.acm.org/doi/10.1145/2063576.2063869
 *
 * 【使用方法】
 *
 * 方法 1：Android WebView 注入（推荐）
 * -----------------------------------
 * Java/Kotlin 代码：
 *
 ...
 *
 * // 注入 Readability 脚本
 * String readabilityJS = readAssetFile("Readability-android.js");
 ...
 *         // 提取文章内容
 *         view.evaluateJavascript("ReadabilityAndroid.parse()", value -> {
 *             JSONObject article = new JSONObject(value);
 *             String title = article.getString("title");
 *             String content = article.getString("content");
 *             String text = article.getString("textContent");
 *             // 处理提取的内容...
 *         });
 *     }
 * });
 ...
 * 1. 在浏览器中打开任意网页
 * 2. 按 F12 打开开发者工具
 * 3. 在 Console 中粘贴此脚本并回车
 * 4. 调用 ReadabilityAndroid.parse() 获取内容
 *
 * 方法 3：作为库引用
 * -----------------------------------
 * <script src="Readability-android.js"></script>
 * <script>
 *   const article = ReadabilityAndroid.parse();
 ...
 * 【API 说明】
 *
 * ReadabilityAndroid.parse([options])
 *
 * 返回对象：
 * {
 *   title: string,        // 文章标题
 ...
 *   length: number,       // 内容字符数
 *   excerpt: string,      // 摘要（前200字符）
 *   byline: string,       // 作者信息
 *   dir: string,          // 文本方向（ltr/rtl）
 *   siteName: string,     // 网站名称
 *   publishedTime: string // 发布时间（如果有）
 * }
 *
 * 可选参数 options：
 * {
 *   debug: boolean,           // 启用调试日志（默认 false）
 *   maxElemsToParse: number,  // 最大解析元素数（默认 0 = 无限制）
 *   nbTopCandidates: number,  // 候选元素数量（默认 5）
 *   charThreshold: number,    // 最小字符数阈值（默认 500）
 *   classesToPreserve: [],    // 要保留的 CSS 类名数组
 *   keepClasses: boolean,     // 保留所有类名（默认 false）
 *   disableJSONLD: boolean    // 禁用 JSON-LD 解析（默认 false）
 * }
 *
 * 【注意事项】
 *
 * 1. 此脚本需要在页面 DOM 完全加载后使用
 * 2. 某些网站可能有反爬虫措施，导致提取失败
 * 3. 动态加载的内容可能需要等待更长时间
 * 4. 建议配合网络请求拦截使用，避免加载不必要的资源
 *
 * 【许可证】
 *
 * Apache License 2.0
...
;(function() {
  ...
  // ========================================================================
  // Readability 核心代码开始
  // 注：以下代码来自 Mozilla Firefox Readability 项目
  // ========================================================================
...
if (typeof module === "object") {
  ...
  // ========================================================================
  // Readability Android - WebView 便捷封装
  // ========================================================================

  /**
   * ReadabilityAndroid 命名空间
   * 提供 Android WebView 使用的便捷接口
   */
  var ReadabilityAndroid = {
     ...
     * @param {number} options.maxElemsToParse - 最大解析元素数
     * @param {number} options.nbTopCandidates - 候选元素数量
     * @param {number} options.charThreshold - 最小字符数阈值
     * @param {boolean} options.keepClasses - 保留所有类名
     * @param {boolean} options.disableJSONLD - 禁用 JSON-LD 解析
    ...
    parse: function(options) {
      try {
        ...
        console.error("Readability parse error:", e.message);
        return null;
      }
    },
     ...
     * 
     * @param {Object} options - 可选配置
     * @returns {string|null} 纯文本内容
     */
    parseText: function(options) {
      var article = this.parse(options);
     ...
     */
    enableReaderMode: function(options) {
      try {
        var article = this.parse(options);
        if (!article) {
        ...
        document.body.innerHTML = `
          <article>
            <h1>${article.title || ''}</h1>
            ${article.byline ? '<p style="color: #666; margin-bottom: 2em;">' + article.byline + '</p>' : ''}
            ${article.content}
          </article>
        `;

        // 注入样式
        document.head.innerHTML = readerCSS + document.head.innerHTML;

        return true;
      } catch (e) {
        console.error("Enable reader mode error:", e.message);
        return false;
      }
    },
     ...
     * 检测页面是否包含可读内容
     * 
     * @returns {boolean} 是否可能包含文章内容
     */
    isReaderable: function() {
      // 简单检测：页面是否有足够的文本内容
      var textLength = document.body.textContent.length;
      var paragraphCount = document.getElementsByTagName('p').length;
      return textLength > 500 && paragraphCount > 2;
    }
  };

  // 导出到全局
  if (typeof window !== 'undefined') {
    window.ReadabilityAndroid = ReadabilityAndroid;
  }
