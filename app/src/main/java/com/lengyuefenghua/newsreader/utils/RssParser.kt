...
class RssParser {
            ...
            while (eventType != XmlPullParser.END_DOCUMENT) {
                if (eventType == XmlPullParser.START_TAG) {
                    ...
                    if (parser.name == "rss" || parser.name == "feed") {
                        ...
                        if (first != null) {
                            LogUtils.log("┍解析文章标题<tag:title>")
                            LogUtils.log("┕${first.title}")

                            LogUtils.log("┍解析文章链接<tag:link>")
                            LogUtils.log("┕${first.url}")

                            LogUtils.log("┍解析文章图片<tag:enclosure/media>")
                            LogUtils.log("┕${first.imageUrl ?: "无"}")

                            LogUtils.log("┍解析文章摘要<tag:description/summary>")
                            LogUtils.log("┕${first.summary.take(50)}...")

                            LogUtils.log("┍解析文章正文<tag:content/encoded>")
                            val contentPreview = first.content?.take(50)?.replace("\n", " ") ?: "无"
                            LogUtils.log("┕$contentPreview")
                        }
    ...
    // RSS <image> 包含 <url>
    private fun readRssImage(parser: XmlPullParser): String? {
        var url: String? = null
        while (parser.next() != XmlPullParser.END_TAG) {
            if (parser.eventType != XmlPullParser.START_TAG) continue
            if (parser.name == "url") {
    ...
    private fun readEntry(parser: XmlPullParser, sourceName: String): Article? {
        ...
        while (parser.next() != XmlPullParser.END_TAG) {
            ...
            when {
                ...
                // 图片提取：enclosure, media:content, media:thumbnail
                tagName == "enclosure" -> {
                    val type = parser.getAttributeValue(null, "type")
                    val url = parser.getAttributeValue(null, "url")
                    if (imageUrl == null && type != null && type.startsWith("image/") && !url.isNullOrBlank()) {
                        imageUrl = url
                    }
                    parser.nextTag()
                }
        ...
        // [新增] 如果还没有图片，尝试从正文或摘要中提取
        if (imageUrl == null) {
            imageUrl = extractImageFromHtml(rawBody)
        }
